// Generated by LiveScript 1.2.0
var spiralize;
spiralize = function(size){
  var currDir, currPos, mov, look, matrix, res$, i$, to$, lresult$, j$, to1$, activateCurrentCell, nextPosition, nextDirection, lookAhead, i, len$, row;
  currDir = 1;
  currPos = [1, 1];
  mov = [[-1, 0], [0, 1], [1, 0], [0, -1]];
  look = [[-2, 0, 0, 0], [0, 2, 0, 0], [0, 0, 2, 0], [0, 0, 0, -2]];
  res$ = [];
  for (i$ = 0, to$ = size + 2; i$ < to$; ++i$) {
    lresult$ = [];
    for (j$ = 0, to1$ = size + 2; j$ < to1$; ++j$) {
      lresult$.push(0);
    }
    res$.push(lresult$);
  }
  matrix = res$;
  activateCurrentCell = function(){
    matrix[currPos[0]][currPos[1]] = 1;
  };
  nextPosition = function(){
    currPos[1] += mov[currDir][1];
    currPos[0] += mov[currDir][0];
  };
  nextDirection = function(){
    if (++currDir === 4) {
      currDir = 0;
    }
  };
  lookAhead = function(){
    if (matrix[currPos[0] + look[currDir][0]][currPos[1]] === 0 && matrix[currPos[0] + look[currDir][2]][currPos[1]] === 0 && matrix[currPos[0]][currPos[1] + look[currDir][1]] === 0 && matrix[currPos[0]][currPos[1] + look[currDir][3]] === 0) {
      return true;
    } else {
      return false;
    }
  };
  for (;;) {
    for (i$ = 0, to$ = size - 1; i$ < to$; ++i$) {
      i = i$;
      if (lookAhead()) {
        activateCurrentCell();
      } else {
        break;
      }
      nextPosition();
    }
    nextDirection();
    if (i <= 1) {
      activateCurrentCell();
      break;
    }
  }
  matrix.pop();
  matrix.shift();
  for (i$ = 0, len$ = matrix.length; i$ < len$; ++i$) {
    row = matrix[i$];
    row.pop();
    row.shift();
  }
  return matrix;
};
console.log(spiralize(5));